...
def prepare_ages_df(df: pd.DataFrame):
    ages_df = df.copy()
...
def split_ages_df_to_train_and_nan(ages_df: pd.DataFrame):
...
def predict_ages(df: pd.DataFrame, age_estimator=None):

    ages_df = prepare_ages_df(df)
    ...
    if age_estimator is None:
        ...
        X_test = ages_test_X.drop(columns=["PassengerId"])

    ages_test_X["Age"] = age_estimator.predict(X_test)

    ages_df["Age"] = ages_df.join(
        ages_test_X["Age"], how="left", lsuffix="_imputed"
    ).apply(lambda x: x["Age_imputed"] if np.isnan(x["Age"]) else x["Age"], axis=1)

    df["Age"] = df.join(ages_df["Age"], how="left", lsuffix="_imputed").apply(
        lambda x: x["Age_imputed"] if np.isnan(x["Age"]) else x["Age"], axis=1
    )

    df["Age"] = df["Age"].map(round)

    return df, age_estimator
...
def main():
    ...
    train_df, age_estimator = predict_ages(train_df)
    test_df, _ = predict_ages(test_df, age_estimator=age_estimator)
